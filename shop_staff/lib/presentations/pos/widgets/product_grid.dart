...
class ProductGrid extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final products = ref.watch(
      posCatalogControllerProvider.select((state) => state.products),
    );
    final loading = ref.watch(
      posCatalogControllerProvider.select((state) => state.loading),
    );
    final favorites = ref.watch(
      posCatalogControllerProvider.select((state) => state.favoriteProductIds),
    );
    final catalog = ref.read(posCatalogControllerProvider.notifier);
                                        ...
                                        child: Material(
                                          ...
                                          child: IconButton(
                                            tooltip: AppLocalizations.of(
                                              context,
                                            ).posPushProductToCustomerTooltip,
                                            icon: const Icon(
                                              Icons.send_rounded,
                                            ),
                                            color: AppColors.amberPrimary,
                                            onPressed: () => customerDisplay
                                                .sendProduct(p, quantity: 1),
                                          ),
                                        ),
                                      ),
                                  ],
